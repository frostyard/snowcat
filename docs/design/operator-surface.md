...
by decision kind** (proposals, blocked, artifact handoffs) rather than one
...
Rendering the progress view is a projection only: it makes no GitHub request
and adds no queue transition. Its proposed and blocked rows reuse the item
page's stale-intent fields and existing same-origin mutation routes, returning
the operator to `/progress`; no other state renders a mutation form. Primary
items are every kind except `pr-review`,
...
own awaiting-import row. Blocked, an expired lease, a predecessor cycle, and a
third-round review decision are amber; a cancelled review satellite and a pull
request closed without merge are red; and an unverified pull request is grey
while GitHub is unavailable. All three tones are stops, so the attention group
collects the grey one too. Attention rows appear once, above repository
groups. A cancelled primary carries no badge and no row at all: it is a
terminal operator decision nobody acts on again, so `readProgress` drops it
the way it drops an aged-out terminal. A completed item with no pull-request
artifact whose kind is one of the catalog's discovery kinds
([`discoveryKinds`](../../src/queue/programs.ts)) is delivered rather than
stalled — it renders at the merged stage with no badge, because a discovery
root delivers by proposing children whose own rows are already on the page.
...
carries a stale `readyToMark`. A pull request with an unresolved or rejected
description handoff also stays off this rail and review adjudication until the
artifact-handoff rail's verify or repair action clears it. Read-only
projection, no GitHub call: the same
...
| `state`, `headSha`, `verifiedAt`, `mergedAt`, description handoff | The artifact's `verification` (`verified` or `unverified`), which `verify-artifacts` refreshes on the host timer; when several completed items report the same pull request, the newest verified source observation supplies state, while any unresolved source or handoff marker remains sticky until its own report is successfully refreshed. A `pr-review-fix` item does not become a reporter or retitle the row, but its marker can pause review and merge |
