...
Fluent is, how it relates to `frostyard/core` and to the repositories it
...
Fluent is a self-hosted work queue that keeps our repositories maintained by
...
```
frostyard/core ──(declares)──► Fluent ──(coordinates)──► workers
     ▲                           │                          │
     │                           ▼                          ▼
...
**Fluent is the coordinator.** It imports core's tree as an atomic, validated
...
repository *enrolled*. Enrollment is a fact Fluent computes, not a switch
anyone flips. Everything Fluent does afterwards is gated on it. Fluent runs on
...
(`make check`, `npm run check`). Fluent reads those; it never overrides them.
...
Codex, Copilot CLI, a person at a terminal — that speaks Fluent's small MCP
contract: `claim_work`, `heartbeat_work`, `complete_work`, `block_work`,
`release_work`. Fluent owns authorization and bookkeeping; the worker owns its
own checkout, credentials, and tools. Fluent never provides a sandbox and never
runs code.
...
1. **Authority flows in from core.** The Core poll fetches core's default
   ...
   whole tree against the bundled schemas (exact byte digests — Fluent will
...
2. **Work flows in from three feeders**, all on an hourly timer, all
   idempotent:
   - **Programs.** Fluent's catalog names a small set of *maintenance
   ...
   - **Issues.** Label a GitHub issue `fluent` and the next feed imports it as
...
5. **Verification is Fluent's.** On completion, Fluent checks every reported
...
6. **Cure keeps pull requests mergeable.** The same pass reads each open pull
   ...
   re-run, reply); Fluent recomputes the patch's identity on completion and
   refuses if it changed. A cure that needs a code change becomes a proposal
   again. Cure never merges, approves, or dismisses.

7. **Merge stays human.** No path in Fluent merges a pull request, cuts a tag,
...
- **Closed vocabularies everywhere.** Actions, program names, boundaries, and
  decisions are enums pinned in core's schemas and Fluent's registry;
...
- **Authority never comes from mutable state.** Not from GitHub topics, not
  ...
  revision that Fluent validated.
- **Workers can propose, not approve.** Follow-ups are proposals; a worker
  ...
  and cannot borrow Fluent's reserved principal namespaces.
- **Facts are computed, not asserted.** Enrollment, delivery, and "was that
  cure mechanical" are things Fluent reads from GitHub and its ledgers, never
...
3. Fluent activates the snapshot, reconciles, and reports it `enrolled`;
   opt it into the queue; the next feed seeds its programs.

Today's fleet: `updex`, `fluent`, `clix`, `std`.

## Giving it work

Label an issue `fluent` and write it so a worker can finish it in one lease
and Fluent can verify the result — the `write-fluent-issues` skill is the
...
- Rationale: [ADR-0059](../adr/0059-adopt-the-queue-store-as-the-v1-work-engine.md)
  ...
  (Fluent owns conformance and triage); core's
