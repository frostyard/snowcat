...
test("re-verify from the browser refreshes a repository's pending artifacts as operator:web and reports counts", async () => {
  ...
  assert.equal(
    ...
    "/?done=artifact.verified&detail=1+checked%2C+1+updated%2C+0+rejected%2C+0+unavailable%2C+0+cure+items+queued%2C+0+review+items+queued%2C+0+marked+ready%2C+0+retired",
  );
...
test("board actions import labeled issues, seed dogfood, verify artifacts, and impose/clear the operator hold as operator:web behind the session", async () => {
  ...
  assert.equal(
    verified.headers.get("Location"),
    "/repositories/frostyard/example?done=artifact.verified&detail=1+checked%2C+1+updated%2C+0+rejected%2C+0+unavailable%2C+0+cure+items+queued%2C+0+review+items+queued%2C+0+marked+ready%2C+0+retired",
  );
