...
test("attachArtifact appends a verified pull request to a completed item so delivery reads merged, once per URL, with one artifact.attached event", async () => {
  ...
  assert.equal(SCHEMA_VERSION, 11, "attaching needs no schema rung of its own: result_json already holds artifacts (rung 5 is the pull-request cure column, rung 6 the repository cure_foreign setting, rung 7 the mcp_tokens table, rung 8 the review gate, rung 9 the token claim restriction, rung 10 the unreported pull-request observation, rung 11 the labeled-issue observation)");
