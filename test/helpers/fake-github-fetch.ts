 ...
 * by `SNOWCAT_TEST_FAKE_GITHUB` — a map from request pathname to
 * `{ status, body }` — and appends every requested pathname to the file named
 * by `SNOWCAT_TEST_FAKE_GITHUB_LOG`. Unlisted paths answer 404. Test-only: the
...
const fixturePath = process.env.SNOWCAT_TEST_FAKE_GITHUB;
if (!fixturePath) throw new Error("SNOWCAT_TEST_FAKE_GITHUB must name the fixture file");
const answers = JSON.parse(readFileSync(fixturePath, "utf8")) as Record<string, FakeAnswer>;
const logPath = process.env.SNOWCAT_TEST_FAKE_GITHUB_LOG;
