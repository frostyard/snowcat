...
// Conformance discovery alone runs the repository's own verify gate (ADR-0043's gate triad) on its detached read-only checkout.
const conformanceDiscoveryActions: AllowedAction[] = ["read", "run-tests", "create-followup"];
...
export const maintenancePrograms: readonly MaintenanceProgram[] = [
  ...
  },
  {
    ...
    discovery: {
      ...
      instructions:
        "Compare the repository with frostyard/core's binding ADRs (its docs/org-adrs.md against Core's current ADR set), the canonical repository surfaces of the surfaces contract (agent instructions, governance file, skills directory, documentation index — present, real content, valid), its `make ci` gate and title lint, and the ACMM criteria (the frostyard-acmm-conformance skill). Identify exactly one current gap; do not treat an aspiration as a binding contract or invent an organization standard. Then run the repository's declared non-mutating gate on this detached read-only checkout — `make verify` where a Makefile declares it, `npm run verify` where package.json declares it — and run `git status --porcelain` afterwards; a non-empty status is the gap (report the exact paths it changed), and an absent gate is itself the gap (ADR-0043's gate triad). Do not edit files or open a GitHub artifact. Propose one bounded child only when justified: a compliance change in this repository, or — when the ADR is what should move — one bounded item to raise it with Core.",
      acceptanceCriteria: [
        ...
        "The result states whether `verify` ran, its exit status, and the `git status --porcelain` output afterwards (empty, or the paths it changed).",
      ...
      allowedActions: conformanceDiscoveryActions,
      // Discovery reads, runs the read-only verify gate, and files follow-ups; it mutates no checkout it keeps (ADR-0073).
      executionTarget: "read-only",
      priority: 0,
    },
  },
