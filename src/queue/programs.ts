...
export const maintenancePrograms: readonly MaintenanceProgram[] = [
  ...
  },
  {
    ...
    discovery: {
      ...
      instructions:
        "Compare the repository with frostyard/core's binding ADRs (its docs/org-adrs.md against Core's current ADR set), the canonical repository surfaces of the surfaces contract (agent instructions, governance file, skills directory, documentation index — present, real content, valid), its `make ci` gate and title lint, and the ACMM criteria (the frostyard-acmm-conformance skill). Identify exactly one current gap; do not treat an aspiration as a binding contract or invent an organization standard. Then run `make verify` on this detached read-only checkout (core ADR-0044: every enrolled repository exposes it — its absence is itself the gap, report it as such and do not substitute another command), then `git status --porcelain`; a non-empty status is the gap (report the exact paths it changed). Do not edit files or open a GitHub artifact. Propose one bounded child only when justified: a compliance change in this repository, or — when the ADR is what should move — one bounded item to raise it with Core.",
      acceptanceCriteria: [
        ...
        "The result states whether `make verify` ran, its exit status, and the `git status --porcelain` output afterwards (empty, or the paths it changed).",
