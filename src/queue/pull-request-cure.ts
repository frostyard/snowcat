...
export function cureRootDefinition(
  ...
  return {
    ...
    instructions: [
      ...
      "If curing this head requires changing the patch (a conflict that needs edits, a failing check that needs a code or test change, a review or review thread that asks for a change), do NOT push: create exactly one existing-pr-change follow-up that names the exact change and how it will be verified; Snowcat supplies the pr-cure-change kind and inherited binding. Block this item if the change is the maintainer's to make.",
