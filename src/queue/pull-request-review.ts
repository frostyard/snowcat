  ...
  return {
    ...
    instructions: [
      ...
      ...(review.originItemId
        ...
        : []),
      `Read the pull request's description and its diff at exactly head ${short} (for example \`gh pr diff ${head.number}\` or \`gh api repos/${repository}/pulls/${head.number}/files\`); check the head out and run the repository's non-mutating gate (\`make verify\`) when it exists; never run \`make check\` or another target that formats or rewrites files, because a review grants no write.`,
...
export function reviewFixRootDefinition(
  ...
  return {
    ...
    instructions: [
      ...
      "After your push the head changes and Snowcat schedules the next review round automatically. Run the repository's own checks locally on the new head before completing (\`make verify\` where it exists).",
