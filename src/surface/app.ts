...
export function createSurfaceApp(options: SurfaceOptions): Hono<SurfaceEnv> {
  ...
  app.post("/repositories/:owner/:name/verify-artifacts", requireConfigured, requireSession, requireSameOrigin, (context) =>
    page(async (stores, enrollments, chrome) => {
      ...
      const detail = `${outcome.checked} checked, ${outcome.updated} updated, ${outcome.rejected} rejected, ${outcome.unavailable} unavailable, ${outcome.cured} cure item${outcome.cured === 1 ? "" : "s"} queued, ${outcome.reviewed} review item${outcome.reviewed === 1 ? "" : "s"} queued`;
