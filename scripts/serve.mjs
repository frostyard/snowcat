#!/usr/bin/env node
// Starts the built Fluent app (`npm run build` → dist/app.mjs) bound to
...
console.log(`[fluent] operator surface listening on http://${hostname}:${port}${hostname === "127.0.0.1" ? " (loopback only; set HOST to bind elsewhere)" : ""}`);
